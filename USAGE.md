...
public class PlayerManager {
    ...
    public void savePlayer(String playerName, int level, double health) throws IOException {
        ICompoundTag player = NBTBuilder.compound("Player")
            .addString("Name", playerName)
            .addInt("Level", level) 
            .addDouble("Health", health)
            .addString("LastLogin", Instant.now().toString())
            .build();
            
        File playerFile = new File("players/" + playerName + ".dat");
        NBTFileFactory.writeNBTFile(playerFile, player, Compression_Types.GZIP);
    }
    
    public PlayerData loadPlayer(String playerName) throws IOException {
        File playerFile = new File("players/" + playerName + ".dat");
        if (!NBTFileFactory.isValidNBTFile(playerFile)) {
            throw new IOException("Invalid player file: " + playerName);
        }
        
        ICompoundTag player = NBTFileFactory.readNBTFile(playerFile);
        return new PlayerData(
            player.getString("Name"),
            player.getInt("Level"),
            player.getDouble("Health"),
            player.getString("LastLogin")
        );
    }
    
    public void updatePlayerLevel(String playerName, int newLevel) throws IOException {
        File playerFile = new File("players/" + playerName + ".dat");
        
        NBTBuilder.fromFile(playerFile)
            .addInt("Level", newLevel)
            .addString("LastLevelUp", Instant.now().toString())
            .buildAndSave(playerFile);
    }
}
```

### Example 2: World Configuration

```java
public class WorldConfig {
    
    public void createWorldConfig(String worldName, int spawnX, int spawnY, int spawnZ) throws IOException {
        NBTBuilder.compound("Data")
            .addString("LevelName", worldName)
            .addInt("SpawnX", spawnX)
            .addInt("SpawnY", spawnY)
            .addInt("SpawnZ", spawnZ)
            .addByte("Difficulty", (byte) 2) // Normal difficulty
            .addByte("hardcore", (byte) 0)   // Not hardcore
            .addList("GameRules", NBTTags.Tag_Compound)
                .addCompound("keepInventory")
                    .addString("Rule", "keepInventory")
                    .addString("Value", "false")
                .end()
                .addCompound("doDaylightCycle")
                    .addString("Rule", "doDaylightCycle")
                    .addString("Value", "true")
                .end()
            .end()
            .buildAndSave(new File("level.dat"), Compression_Types.GZIP);
    }
    
    public void updateGameRule(String rule, String value) throws IOException {
        File levelFile = new File("level.dat");
        ICompoundTag level = NBTFileFactory.readNBTFile(levelFile);
        
        IListTag gameRules = level.getList("GameRules");
        if (gameRules != null) {
            // Find and update existing rule or add new one
            boolean found = false;
            for (ITag<?> ruleTag : gameRules.getData()) {
                if (ruleTag instanceof ICompoundTag ruleCompound) {
                    if (rule.equals(ruleCompound.getString("Rule"))) {
                        ruleCompound.addString("Value", value);
                        found = true;
                        break;
                    }
                }
            ...
            if (!found) {
                gameRules.addCompound(rule)
                    .addString("Rule", rule)
                    .addString("Value", value);
            }
        }
        
        NBTFileFactory.writeNBTFile(levelFile, level);
    }
}
```
...
public class InventoryManager {
    ...
    public void createPlayerInventory(String playerName) throws IOException {
        NBTBuilder.compound("Player")
            ...
            .addList("Inventory", NBTTags.Tag_Compound)
                .addCompound("slot0")
                    ...
                    .addShort("Damage", (short) 0)
                    .addByte("Slot", (byte) 0)
                .end()
                .addCompound("slot1")
                    .addString("id", "minecraft:bread")
                    .addByte("Count", (byte) 32)
                    .addByte("Slot", (byte) 1)
                .end()
            .end()
            .buildAndSave(new File("inventories/" + playerName + ".dat"));
    }
    
    public void addItemToInventory(String playerName, String itemId, int count, int slot) throws IOException {
        File inventoryFile = new File("inventories/" + playerName + ".dat");
        ICompoundTag player = NBTFileFactory.readNBTFile(inventoryFile);
        
        IListTag inventory = player.getList("Inventory");
        if (inventory != null) {
            inventory.addCompound("slot" + slot)
                .addString("id", itemId)
                .addByte("Count", (byte) count)
                .addByte("Slot", (byte) slot);
        }
        
        NBTFileFactory.writeNBTFile(inventoryFile, player);
    }
    
    public List<ItemStack> getInventoryItems(String playerName) throws IOException {
        File inventoryFile = new File("inventories/" + playerName + ".dat");
        ICompoundTag player = NBTFileFactory.readNBTFile(inventoryFile);
        ...
        if (inventory != null) {
            for (ITag<?> itemTag : inventory.getData()) {
                if (itemTag instanceof ICompoundTag itemCompound) {
                    items.add(new ItemStack(
                        itemCompound.getString("id"),
                        itemCompound.getByte("Count"),
...
public class NBTBatchProcessor {
    ...
    public void convertCompressionFormat(File sourceDir, File targetDir, Compression_Types newCompression) {
        File[] nbtFiles = sourceDir.listFiles((dir, name) -> name.endsWith(".dat"));
        
        if (nbtFiles != null) {
            for (File sourceFile : nbtFiles) {
                try {
                    if (NBTFileFactory.isValidNBTFile(sourceFile)) {
                        ICompoundTag data = NBTFileFactory.readNBTFile(sourceFile);
                        File targetFile = new File(targetDir, sourceFile.getName());
                        NBTFileFactory.writeNBTFile(targetFile, data, newCompression);
                        System.out.println("Converted: " + sourceFile.getName());
                    }
                } catch (IOException e) {
                    System.err.println("Failed to convert " + sourceFile.getName() + ": " + e.getMessage());
                }
            }
        }
    }
    
    public void validateAllFiles(File directory) {
        File[] nbtFiles = directory.listFiles((dir, name) -> name.endsWith(".dat"));
        
        if (nbtFiles != null) {
            int valid = 0, invalid = 0;
            
            for (File file : nbtFiles) {
                if (NBTFileFactory.isValidNBTFile(file)) {
                    valid++;
                    System.out.println("✓ " + file.getName());
                } else {
                    invalid++;
                    System.out.println("✗ " + file.getName() + " (corrupted or invalid)");
                }
            }
            
            System.out.printf("Validation complete: %d valid, %d invalid%n", valid, invalid);
        }
    }
    
    public void backupWithMetadata(File sourceDir, File backupDir) throws IOException {
        File[] nbtFiles = sourceDir.listFiles((dir, name) -> name.endsWith(".dat"));
        ...
        if (nbtFiles != null) {
            for (File sourceFile : nbtFiles) {
                ...
                NBTFileFactory.copyNBTFile(sourceFile, backupFile);
                
                // Create metadata file
                ICompoundTag metadata = NBTFactory.createCompound("BackupMetadata")
                    .addString("OriginalPath", sourceFile.getAbsolutePath())
                    .addString("BackupDate", Instant.now().toString())
                    .addLong("OriginalSize", sourceFile.length())
                    .addString("Checksum", calculateChecksum(sourceFile));
                
                File metadataFile = new File(backupDir, sourceFile.getName() + ".meta");
                NBTFileFactory.writeNBTFile(metadataFile, metadata, Compression_Types.NONE);
            }
        }
    }
