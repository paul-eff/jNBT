...
public class TAG_Int_Array
{
    public static final int ID = 11;
    public static final String NAME = "TAG_Int_Array";
     ...
     * @return
     */
    private static int[] readTagPayload(byte[] nbtByteArray, int currentPosition, int nameLength)
    {
        ...
        int[] intArray = new int[PAYLOAD_BYTE_SIZE];
        for (int i = 0; i < PAYLOAD_BYTE_SIZE; i++)
        {
            byteArr[0] = nbtByteArray[currentPosition + 1 + NAME_BYTE_LENGTH + nameLength + PREFIX_BYTE_LENGTH + (i * 4)];
            byteArr[1] = nbtByteArray[currentPosition + 1 + NAME_BYTE_LENGTH + nameLength + PREFIX_BYTE_LENGTH + (i * 4)];
            byteArr[2] = nbtByteArray[currentPosition + 1 + NAME_BYTE_LENGTH + nameLength + PREFIX_BYTE_LENGTH + (i * 4)];
            byteArr[3] = nbtByteArray[currentPosition + 1 + NAME_BYTE_LENGTH + nameLength + PREFIX_BYTE_LENGTH + (i * 4)];

            intArray[i] = ByteBuffer.wrap(byteArr).order(ByteOrder.BIG_ENDIAN).getInt();
        }
        // Currently PAYLOAD_BYTE_SIZE is holding number of int not of actual bytes needed
